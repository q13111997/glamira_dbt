WITH stg_sales_order__source AS (
    SELECT *
    FROM {{ source('glamira_raw', 'summary') }}
    WHERE collection = 'checkout_success'
),
stg_sales_order__unnest AS (
    SELECT
    ...
    FROM stg_sales_order__source
    CROSS JOIN UNNEST(cart_products) AS cp
), 
-- Chuẩn hóa dấu ngăn cách thập phân ở cột unit_price + join thêm bảng tỷ giá quy ra USD
stg_sales_order__cast_price AS (
    SELECT
        COALESCE(t1.user_id_db,-1) AS user_id_db
        ,t1.email_address
        ,t1.ip
        ,t1.date_id
        ,t1.store_id
        ...
        ,rate.rate_to_usd
    FROM stg_sales_order__unnest t1
...
stg_sales_order__customer_per_order AS (
    SELECT
        order_id
        ,user_id_db
        ,email_address
    ...
    FROM stg_sales_order__cast_price
), 
-- Lấy đơn hàng và mã khách hàng đầu tiên trong đơn hàng
stg_sales_order__order_customer AS (
    SELECT
        order_id
        ,user_id_db
        ,email_address
        ...
        ,date_id
        ,order_ts_utc
    FROM stg_sales_order__customer_per_order
    WHERE rn = 1
), 
...
stg_sales_order__order_product AS (
    SELECT
        order_id
        ,product_id
        ,currency_code
...
SELECT
    ...
    ,op.quantity
    ,op.revenue
FROM stg_sales_order__order_product op
JOIN stg_sales_order__order_customer oc USING (order_id)
